/* ==========================================================
   SOSEEKS FASTAPI ACADEMY — App Engine
   ========================================================== */

const ACCESS_CODE = 'FASTAPI2026';
const STORAGE_KEY = 'soseeks_fastapi';

class FastAPIAcademy {
    constructor() {
        this.progress = {};
    ...
    showDashboard() {
        ...
        container.innerHTML = `
        <div class="dash-hero">
            <h1>Fast<span class="glow">API</span></h1>
            <p>Master FastAPI from fundamentals to production deployment — with real-world analogies, animated examples, and interview-ready quizzes.</p>
        </div>
        <div class="dash-grid">
            ${MODULES.map(mod => {
        ...
        switch (analogy.type) {
            ...
            case 'envelope':
                return `<div style="text-align:center">
                    <div style="display:inline-block;padding:16px 28px;border:2px solid var(--warning);border-radius:4px;position:relative;background:rgba(245,158,11,0.06)">
                        <div style="font-family:var(--font-code);font-size:0.85rem;color:var(--warning)">${analogy.content || '{ data }'}</div>
                    ...
                    <div style="font-size:0.75rem;color:var(--text-dim);margin-top:10px">${analogy.sublabel || 'Validated & sealed by Pydantic'}</div>
                </div>`;
            ...
            case 'blueprint':
                return `<div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap;justify-content:center">
                    <div style="padding:16px 24px;border:2px dashed var(--primary);border-radius:12px;text-align:center">
                        ...
                        <div style="font-family:var(--font-code);font-size:0.85rem">${analogy.className || 'Model'}</div>
                    </div>
                    <span class="machine-arrow">→</span>
            ...
            case 'diary':
                return `<div style="text-align:center">
                    <div style="display:inline-block;padding:20px 28px;background:rgba(99,102,241,0.04);border:1px solid var(--border);border-radius:4px 12px 12px 4px;border-left:4px solid var(--primary);min-width:200px;text-align:left">
                        <div style="font-size:0.7rem;color:var(--primary-light);margin-bottom:8px">📖 ${analogy.filename || 'Dockerfile'}</div>
                        <div style="font-family:var(--font-code);font-size:0.8rem;color:var(--text-muted)">
                            ${analogy.lines || 'Step 1<br>Step 2'}
                        </div>
                    </div>
                </div>`;
            ...
            case 'waiter':
                return `<div class="anim-chefs">
                    ${(analogy.waiters || [
                        { emoji: '🧑‍🍳', task: 'cooking...' },
                        { emoji: '🧑‍💼', task: 'taking orders' },
                        { emoji: '🧑‍💼', task: 'serving' }
                    ]).map((w, i) =>
                        `<div class="chef-lane" style="border-color:${['var(--primary)', 'var(--success)', 'var(--warning)'][i % 3]}">
                            <div class="chef-emoji">${w.emoji}</div>
                            <div class="chef-task" style="--d:${i * 0.4}s">${w.task}</div>
                        </div>`
                    ).join('')}
                </div>`;
